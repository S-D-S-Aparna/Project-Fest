...
export default function CommunityHub() {
  ...
  const fetchPosts = async () => {
    try {
      const response = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/community/posts`);
  ...
  const handleCreatePost = async () => {
    ...
    try {
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/community/posts`,
        {

...
export default function RoadmapPage() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/roadmaps`, {
        headers: { Authorization: `Bearer ${token}` }
      })
      .then((res) => {
        if (active) {
    ...
    <MainLayout>
      <div className="max-w-4xl mx-auto">
        ...
        {roadmaps.map((rm) => {
          ...
          try {
            milestones = JSON.parse(rm.milestones) as Milestone[];
          } catch (error) {
            console.error("Failed to parse milestones for", rm.id, error);
          }
      ...
      <div className="bg-purple-50 border border-purple-100 rounded-2xl p-8 text-center mb-8 max-w-4xl mx-auto">
        ...
        <div className="flex justify-center gap-4">
           <Link href="/resources" className="bg-purple-600 text-white px-6 py-2.5 rounded-lg font-medium hover:bg-purple-700 transition-colors shadow-sm text-sm">
             Explore Resources
           </Link>
           <Link href="/mentors" className="bg-white border border-gray-200 text-gray-700 px-6 py-2.5 rounded-lg font-medium hover:bg-gray-50 transition-colors text-sm">
             Talk to a Mentor
           </Link>
        </div>
      </div>
